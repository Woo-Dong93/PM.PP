...
#b_table table input:hover {
	height: 19px;
	background: #fff;
}

#b_table table textarea {
	border: 1px solid #d5d2d2;
	background: #f6f6f6
}

#b_table table textarea:hover {
	border: 1px solid #d5d2d2;
	background: #fff;
}
...
<body>
	<%
		...
		Bbs bbscontent = null;
//등록
		if (request.getParameter("order") != null && request.getParameter("order").equals("insert")) { //밑에서 order = insert값을 전송하면
			Bbs bbs = new Bbs();
			System.out.println(session.getAttribute("idx").toString()); //테스트하기위해 
			...
			dbmManager.insertBbs(request.getParameter("bbs_name"), bbs); //db에 게시판글등록
			
			%>
			<script>document.location = "p-borad.jsp"</script> 
			<%
		}
		//삭제
		if(request.getParameter("order") != null && request.getParameter("order").equals("del")){
			...
			%>
			<script>alert('삭제하였습니다.');document.location = "p-borad.jsp"</script>
			<%
		}
		//예외처리
	...
	<div id="wrapper">
			...
			<div id="header" class="container">
				...
				<div id="menu">
					<ul>
						...
						<li class="current_page_item"><a href="p-borad.jsp"
							accesskey="3" title="">P-board</a></li>
						<li class="m1"><a href="free-borad.jsp" accesskey="4"
							title="">Free board</a></li>
		...
		<div id="contents">
			<form name="bbs_form" id="bbs_form" method="post">
				<input name="order" type="hidden" value="" id="order"> <input
					name="bbs_name" type="hidden" value="bbs_party"> <img
					src="picture/blogo.png" alt="오류" id="blogo">
				<div id="contents">
					<div id="b_table">
						<table cellpadding="0" cellspacing="0">
							<caption>파티원 구하기(글쓰기)</caption>
							<tbody>
								<tr>
									<th
										style="background: #f7f7f7; color: #000; padding: 10px; width: 50px;">제목</th>
									<td style="background: #f7f7f7; color: #000;"><input
										type="text" value="<%=bbscontent.getTitle()%>" name="title"
										style="width: 99%;" /></td>
								</tr>
								<tr>
									<td colspan="2" style="background: #f7f7f7; color: #000;"><img
										src="images/add.gif" alt="" /></td>
								<tr>
								<tr>
						...
						<ul class="btn">
							...
							<li class="btn_middle"><span><a href="p-borad.jsp">목록</a></span></li>
							<li class="btn_right"></li>
						</ul>
						<ul class="btn">
							<li class="btn_left"></li>
							<li class="btn_middle"><span><a href="#" onclick="del()">삭제</a></span></li>
							<li class="btn_right"></li>
						</ul>
						<!-- 
						<ul class="btn">
							<li class="btn_left"></li>
							<li class="btn_middle"><span><a href="#">수정</a></span></li>
							<li class="btn_right"></li>
						</ul>
						 -->
						<ul class="btn">
							<li class="btn_left"></li>
							<li class="btn_middle"><span><a href="#" onclick="insert()">글쓰기</a></span></li>
							<li class="btn_right"></li>
						</ul>
						<!--btn_End-->
					</div>
